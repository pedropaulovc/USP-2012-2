...
public class FilmeInfo implements Comparable<FilmeInfo> {
	...
	private static int hourParser(String hora) {
		...
		int tempo;
		try {
			tokens = hora.split(delims);
			tempo = (Integer.parseInt(tokens[0]) * 60);
		} catch (Exception e) {
			return 0;
		}
		if(tokens.length > 2)
	...
	public String toString() {
		Calendar released = getReleased();
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
		String releasedFormat = sdf.format(released.getTime());
		
		return "FilmeInfo [Actors=" + getActors() + ", Director=" + getDirector()
				+ ", Genre=" + getGenre() + ", Plot=" + getPlot() + ", Poster=" + getPoster()
				+ ", Rated=" + getRated() + ", Released=" + releasedFormat + ", Response="
